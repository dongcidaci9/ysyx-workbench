...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
	...
	//INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(rd) = BITS(SEXT(src1 + imm, 32), 31, 0)); 
	//INSTPAT("0000000 ????? ????? 000 ????? 01100 11", addw   , R, R(rd) = BITS(SEXT(src1 + src2, 32), 31, 0));
	INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(rd) = src1 & src2);
	INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , R, R(rd) = src1 & imm);
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(rd) = s->pc + (BITS(imm, 32, 13) << 12));
  ...
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(rd) = src1 | src1);
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sl     , S, R(rd) = src1 << src2);
  ...
  //INSTPAT("??????? ????? ????? 011 ????? 01000 11", sd     , S, Mw(src1 + imm, 8, src2));
	...
	INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (word_t)src1 >> BITS(imm, 5, 0)); 
	...
	INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , R, R(rd) = src1 ^ src1);
